...
import { CloseCircleOutlined, ReloadOutlined, SearchOutlined, UsergroupAddOutlined } from "@ant-design/icons";
import { Avatar, Button, Col, Input, Modal, Row, Space, Tabs, Tooltip, message } from "antd";
...
const ChatLayoutHome = ({ children }) => {
    ...
    const [isProfileDrawerVisible, setIsProfileDrawerVisible] = useState<boolean>(false);

    const [isCreateNewGroupVisible, setIsCreateNewGroupVisible] = useState<boolean>(false);
    const [isContactInfoDrawerVisible, setIsContactInfoDrawerVisible] = useState<boolean>(false);
    const { joinRoom, conversations, sendMessage, setCurrentRoom, currentRoom, notifyMessageOfImagesData, showModal, setShowModal, approveImagePermission, messageApprovalStatus } = useSocket();
    ...
    const handleApproveImagePermission = () =>{
        ...
        approveImagePermission(currentRoom?.roomId, notifyMessageOfImagesData?.senderID, notifyMessageOfImagesData?.recieverId)
    ...
    return (
        <div style={{ boxShadow: "0 0 10px rgba(0,0,0,0.1)" }}>
                ...
                <Col
                    ...
                    <Space
                        ...
                        <Avatar
                            ...
                            {get(user, "name", "")?.charAt(0).toUpperCase() || get(user, "nickName", "")?.charAt(0).toUpperCase()}
                        </Avatar>
                        <Space>
            ...
            <LeaveGroupModal leaveGroupModal={leaveGroupModal} setLeaveGroupModal={setLeaveGroupModal} />
            {
                showModal ? notifyMessageOfImagesData?.senderID === user?._id ? <>

                    <Modal title="Basic Modal" open={showModal} onOk={handleOk} onCancel={handleCancel}>
                        <p>Wait for Approval</p>
                  
                    </Modal>
                </> : <Modal open={showModal} onOk={handleApproveImagePermission} onCancel={handleCancel}>
                    <p>Please Approve</p>
                </Modal> : null
            }
            {
                messageApprovalStatus ? user?.status === "pending" &&
                 <>

                    <Modal title="Basic Modal" open={messageApprovalStatus} onOk={handleOk} onCancel={handleCancel}>
            ...
            {
                messageApprovalStatus ? user?.status === "active" &&
                 <>
